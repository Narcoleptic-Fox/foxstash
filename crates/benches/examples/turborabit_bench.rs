...
fn main() {
    ...
    let rerank: usize = if storage == Storage::F32 {
        0
    } else {
        env("RERANK", "200").parse().unwrap()
    };

    let train_flat = read_f32(&format!("{dir}/train.bin"));
    let queries: Vec<Vec<f32>> = read_f32(&format!("{dir}/test.bin"))
        .chunks_exact(dim)
        .map(|r| r.to_vec())
        .collect();
    let gt = read_i32(&format!("{dir}/gt.bin"));
    let truth: Vec<HashSet<usize>> = gt
        .chunks_exact(gtk)
        .map(|r| r.iter().take(k).map(|&i| i as usize).collect())
        .collect();
    ...
    if std::env::var("REORDER").is_ok() {
        let t0 = Instant::now();
        index = index.reorder_for_locality();
        println!(
            "reordered (BFS locality) {:.1}s",
